...
def main(args):
    ...
    set_variable0,set_variable1=set_privacy_budget,set_num_epochs
    final_test_loss = [[0 for i in range(len(set_variable0))] for j in range(len(set_variable1))]
    final_test_acc = [[0 for i in range(len(set_variable0))] for j in range(len(set_variable1))]
    for eps in range(len(set_privacy_budget)):
        ...
        for j in range(len(set_num_epochs)):
            ...
            final_test_loss[j][eps] = copy.deepcopy(sum(avg_obj)/len(avg_obj))
            final_test_acc[j][eps] = copy.deepcopy(sum(avg_acc)/len(avg_acc))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_epochs', type=list, default=range(10,105,5))
    parser.add_argument('--reg_coeff', type=float, default=0.001)
    parser.add_argument('--batch_size', type=int, default=16)
    parser.add_argument('--num_users', type=int, default=50)
    parser.add_argument('--num_Chosenusers', type=int, default=30)    
    parser.add_argument('--num_train', type=int, default=128)
    parser.add_argument('--num_experiments', type=int, default=20)
    parser.add_argument('--delta', type=float, default=0.01)
    parser.add_argument('--set_privacy_budget', type=int, default=range(10,32,2))
